...
class App extends React.Component{
  ...
  async componentDidMount(){
      ...
      // await fetch('https://covid19api.com/#subscribe%22,%22success%22:false')
  ...
  render(){
    ...
    return(
        <div className="App">
          <p className='headerText'>Covid19 stats web App</p>
          <SearchBox placeholder="Enter Country Name .." handleChange={this.handleChange} />
          <CountryList stats = {filteredCountries} />
          <p className='footer'>By <code>WONDETOM</code> on {new Date().toLocaleString()}</p>
        </div>
      )
  }
}
